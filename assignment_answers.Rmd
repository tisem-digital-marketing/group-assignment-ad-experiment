...
```{r}
# why? advertising doesnt always have immediate effect
reg_data <-
    ad_experiment %>%
    filter(test_period %in% c('test', "post-test")) %>% #== "test") %>%
    group_by(geo) %>%
    summarise(sales = sum(sales),
              cost = sum(cost),
              # keeps treatment status
              treatment_status = min(treatment_status)
              )

# 1 point regression + results
tidy(lm(log(sales) ~ treatment_status, data = reg_data))
```
...
cuped_data <-
    ad_experiment %>%
    filter(test_period %in% c("pre_test", 'test')) %>%
    group_by(geo, test_period) %>%
    summarise(sales = sum(sales),
              treatment_status = min(treatment_status)
              ) %>%
    ungroup() %>%
    pivot_wider(names_from=test_period, values_from = sales)

# (1pt)
theta <-
    tidy(lm(test ~ pre_test, data = cuped_data)) %>%
    filter(term == "pre_test") %>%
    pluck("estimate")
...
cuped_data <-
    cuped_data %>%
    mutate(sales_cuped = test - theta*(pre_test - mean(cuped_data$pre_test)))

# (1 pt)
tidy(lm(log(sales_cuped) ~ treatment_status, data = cuped_data))
```
...
```{r}
# remark: needs to an aggregate level, not at the market level
roi_point <-
    ad_experiment %>%
    filter(test_period == "test") %>%
    # for each geo get sales and cost
    group_by(geo) %>%
    summarise(
        sales = sum(sales),
        cost = sum(cost),
    ...
    # average over treatment status -- 
    # average is empirical expectation
    group_by(treatment_status) %>%
    summarise(sales = mean(sales),
              cost  = mean(cost)
    ) %>%
    mutate(incr_revenue = sales - lag(sales),
           incr_cost = cost - lag(cost)) %>%
    slice(n()) %>%
    mutate(roi = incr_revenue / incr_cost)
```
